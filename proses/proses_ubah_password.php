...
if (!empty($_POST['ubah_password_validate'])) {
    $query = mysqli_query($conn, "SELECT * FROM tb_userc WHERE username = '$_SESSION[username_cafeku]' && password = '$passwordlama'");
    $hasil = mysqli_fetch_array($query);
    if ($hasil) {
        if ($passwordbaru == $repasswordbaru) {
            $query = mysqli_query($conn, "UPDATE tb_userc SET password='$passwordbaru' WHERE username = '$_SESSION[username_cafeku]'");
            if ($query) {
